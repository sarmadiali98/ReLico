...
public class LinguaFrancaCodeGenerator {
    ...
    private static class ConstructorInfo {
        ...
        Map<String, String> paramToStateMapping = new HashMap<>();
        List<Statement> constructorBodyStatements = new ArrayList<>();
        // ==============================================
    }
    ...
    public void generateCode(String outputPath) {
        ...
        if (rebecaModel.getRebecaCode().getEnvironmentVariables() != null) {
            for (FieldDeclaration fd : rebecaModel.getRebecaCode().getEnvironmentVariables()) {
                for (VariableDeclarator vd : fd.getVariableDeclarators()) {
                    ...
                    String varName = vd.getVariableName();
                    String literalValue = null;
                    org.rebecalang.compiler.modelcompiler.corerebeca.objectmodel.VariableInitializer initializer = vd.getVariableInitializer();

                    if (initializer instanceof org.rebecalang.compiler.modelcompiler.corerebeca.objectmodel.OrdinaryVariableInitializer oval) {
                        Expression exprValue = oval.getValue();
                        String lfType = mapRebecaTypeToLF(fd.getType().getTypeName());
                        literalValue = expressionToString(exprValue, lfType, getDefaultValueForType(lfType));
                    } else if ("Literal".equals(initializer != null ? initializer.getClass().getSimpleName() : "")) {
                        // Attempt to get literal value directly, avoiding reflection
                        java.lang.reflect.Method getLiteralValueMethod = Arrays.stream(initializer.getClass().getMethods())
                                .filter(m -> m.getName().equals("getLiteralValue"))
                                .findFirst()
                                .orElse(null);

                        literalValue = (getLiteralValueMethod != null) ? initializer.toString() : null;
                    } else if (initializer != null) {
                        literalValue = initializer.toString();
                    }

                    if (literalValue == null) {
                        String lfType = mapRebecaTypeToLF(fd.getType().getTypeName());
                        literalValue = getDefaultValueForType(lfType);
                    }

                    envVariables.put(varName, literalValue);
                }
            }
        }
        ...
        // 4) Classify which msgSrvs are internal vs external
        Map<String, Set<String>> internalMsgsrvs = findInternalMsgServers(constructorInfoMap);

        // 5) Build instance->class map and known rebec info
        ...
        // 8) Generate one reactor per Rebeca class
        for (ReactiveClassDeclaration rc : classes) {
            String className = rc.getName();
            ConstructorInfo cInfo = constructorInfoMap.get(className);
            ...
            if (!unusedServers.isEmpty()) {
                lfCode.append("\n");
                for (String m : unusedServers) {
                    MsgsrvDeclaration mDecl = findMsgsrv(rc, m);
                    if (mDecl != null) {
                        // Generate reaction body for documentation but no ports or live traces
                        ReactionContent rContent = generateUnusedMsgsrvReactionBody(
                                mDecl, className, cInfo, instanceToClass, callsByInstance
                        );

                        lfCode.append("    // Unused msgsrv: ").append(m).append("\n").append
                                ("    // This reaction is declared but never used\n").append
                                ("    // Since it is neither internal nor external, it's not triggered by a logical action or an input\n").append
                                ("    // This will cause a warning you can either safely ignore,\n").append
                                ("    // or remove the message server from your source Timed Rebeca code\n\n")
                        ;
                        ...
                        if (!rContent.code.isEmpty()) {
                            ...
                            if (!rContent.usedPorts.isEmpty()) {
                                lfCode.append(" -> ");
                                lfCode.append(String.join(", ", rContent.usedPorts));
                            }
                            lfCode.append(" {=\n");
                            lfCode.append(rContent.code);
                            lfCode.append("    =}\n");
        ...
        // 10) POST-PROCESSING: Eliminate duplicate .set() and .schedule() calls
        String generatedCode = lfCode.toString();
        SimpleBroadcastEliminator eliminator = new SimpleBroadcastEliminator();
        String cleanedCode = eliminator.eliminateDuplicates(generatedCode);
    ...
    /**
     * Analyzes and prints a structure map for debugging (optional).
     */
    ...
    /**
     * Collects all message servers (including constructors) declared by each class.
     */
    ...
    /**
     * Prints a high-level structure map of rebecs, known rebecs, and calls made.
     */
    ...
    /* ----------------------------------------------------------------------
     * Internal Helper Methods for Code Generation
     * ----------------------------------------------------------------------
     */
    ...
    /**
     * Find msgsrvs that are declared but never called (neither internally nor externally).
     */
    private Map<String, Set<String>> findUnusedMsgServers(
            Map<String, Set<String>> internalMsgsrvs,
            Map<String, List<CallDetail>> callsByInstance,
            Map<String, String> instanceToClass
    ) {
        Map<String, Set<String>> unusedMsgsrvs = new HashMap<>();

        for (ReactiveClassDeclaration rc : classes) {
            String className = rc.getName();
            Set<String> unused = new HashSet<>();

            // All declared msgsrvs of this class
            Set<String> allDeclared = new HashSet<>();
            for (MsgsrvDeclaration m : rc.getMsgsrvs()) {
                allDeclared.add(m.getName());
            }

            // Internal msgsrvs of this class
            Set<String> internals = internalMsgsrvs.getOrDefault(className, Collections.emptySet());

            // External msgsrvs of this class (called by other rebecs)
            Set<String> externals = new HashSet<>();
            for (List<CallDetail> callList : callsByInstance.values()) {
                for (CallDetail cd : callList) {
                    if (!cd.isInternal && cd.externalTargetInstance != null) {
                        String targetClass = instanceToClass.get(cd.externalTargetInstance);
                        if (className.equals(targetClass)) {
                            externals.add(cd.msgName);
                        }
                    }
                }
            }

            // Unused = declared but neither internal nor external
            for (String msgsrvName : allDeclared) {
                if (!internals.contains(msgsrvName) && !externals.contains(msgsrvName)) {
                    unused.add(msgsrvName);
                }
            }

            unusedMsgsrvs.put(className, unused);
        }

        return unusedMsgsrvs;
    }
    ...
    /**
     * Generate the reaction content for an internal msgsrv.
     */
    ...
    private ReactionContent generateConstructorReactionBody(
        ...
        // Process only filtered statements from cInfo
        if (cInfo.constructorBodyStatements.isEmpty()) {
            return rc;
        }

        StringBuilder code = new StringBuilder();
        for (Statement stmt : cInfo.constructorBodyStatements) {
            String translated = parseBlockStatementInline(
                    stmt,              // Process each filtered statement
                    className,
                    "",
                    cInfo,
                    instanceToClass,
    ...
    /**
     * Convert Rebeca statements into lines of LF code for a reaction body (inline).
     */
    ...
    private String processExpressionInline(
        ...
        if (expr instanceof BinaryExpression be) {
                    ...
                    for (Map.Entry<String, List<CallDetail>> e : callsByInstance.entrySet()) {
                        ...
                        for (CallDetail cd : e.getValue()) {
                            if (!cd.isInternal && cd.msgName.equals(methodName)
                                ...
                                List<FormalParameterDeclaration> fps = (calleeCls == null)
                                        ? Collections.emptyList()
                                        : cInfoFor(calleeCls, instanceToClass)
                                        .msgsrvToParams
                                        .getOrDefault(methodName, Collections.emptyList());

                                String structType = (fps.isEmpty()) ? "int" : (calleeCls + "_" + methodName);
            ...
            // no-op for direct literal usage
        } else if (expr instanceof TermPrimary) {
            // no-op for simple variable references without assignment/calls
        }
        return sb.toString();
    }
    ...
    private ConstructorInfo buildConstructorInfo(ReactiveClassDeclaration rc) {
        ...
        // Collect all state var names and types
        ...
        // Separate constructor-params from normal state vars
        if (cDec == null) {
            ...
            cInfo.paramToStateMapping = param2stateVar;

            // Build set of state variable names for filtering
            Set<String> stateVarNames = new HashSet<>(statevarNameToType.keySet());

            // Filter constructor body: skip param-to-state assignments
            if (cDec.getBlock() != null && cDec.getBlock() instanceof BlockStatement) {
                BlockStatement block = (BlockStatement) cDec.getBlock();
                for (Statement stmt : block.getStatements()) {
                    // Skip assignments like: id = _id
                    if (!isParameterToStateAssignment(stmt, param2stateVar, stateVarNames)) {
                        cInfo.constructorBodyStatements.add(stmt);
                    }
                }
            }
        ...
        // Map each msgsrv name to its formal parameters
    ...
    /**
     * Checks if a statement is a parameter-to-state assignment (e.g., id = _id).
     * Returns true if this assignment should be skipped in LF translation.
     */
    private boolean isParameterToStateAssignment(Statement stmt,
                                                 Map<String, String> paramToState,
        ...
        // Check if it's a binary expression with "=" operator
        if (expr instanceof BinaryExpression be) {
            if ("=".equals(be.getOperator())) {
                // Extract left side (should be state variable)
                String lhs = null;
                if (be.getLeft() instanceof TermPrimary tp) {
                    lhs = tp.getName();
                }

                // Extract right side (should be constructor parameter)
                String rhs = null;
                if (be.getRight() instanceof TermPrimary tp) {
                    rhs = tp.getName();
                }

                // Skip if: stateVar = constructorParam (e.g., id = _id)
                if (lhs != null && rhs != null &&
                        stateVars.contains(lhs) && paramToState.containsKey(rhs)) {
                    return true;
                }
            }
        }
    ...
    /**
     * Parse a Rebeca expression to detect calls and store them in callsByInstance.
     */
    private void parseExpression(
        ...
        if (expr instanceof DotPrimary dot) {
            parseExpression(dot.getLeft(), currentClass, instanceToClass, callsByInstance, classToKnownRebecs, instanceKnownRebecs, currentMsgsrvName);
            parseExpression(dot.getRight(), currentClass, instanceToClass, callsByInstance, classToKnownRebecs, instanceKnownRebecs, currentMsgsrvName);
                ...
                if (psp instanceof TimedRebecaParentSuffixPrimary timedPSP) {
                    ...
                    if (afterExpr != null) {
                        ...
                        for (Map.Entry<String, String> entry : envVariables.entrySet()) {
                            afterDelayMs = afterDelayMs.replace(entry.getKey(), entry.getValue());
                        }
                    }
                }
                ...
                if (dot.getLeft() instanceof TermPrimary leftTerm && "self".equals(leftTerm.getName())) {
                    ...
                    if (classToKnownRebecs
                        ...
                        for (Map.Entry<String, String> e : instanceToClass.entrySet()) {
                            ...
                            String callerInstance = e.getKey();
                            List<String> knownList = instanceKnownRebecs.getOrDefault(callerInstance, Collections.emptyList());
                            int idx = classToKnownRebecs.get(currentClass).indexOf(knownField.getName());
            ...
            parseExpression(ue.getExpression(), currentClass, instanceToClass, callsByInstance, classToKnownRebecs, instanceKnownRebecs, currentMsgsrvName);
        } else if (expr instanceof BinaryExpression be) {
            parseExpression(be.getLeft(), currentClass, instanceToClass, callsByInstance, classToKnownRebecs, instanceKnownRebecs, currentMsgsrvName);
            parseExpression(be.getRight(), currentClass, instanceToClass, callsByInstance, classToKnownRebecs, instanceKnownRebecs, currentMsgsrvName);
        }
    }
    ...
    private String transformExpression(
        ...
        if (expr instanceof TermPrimary tp) {
            ...
            // If 'raw' is an environment variable, inline its literal value with a comment.

            if (envVariables.containsKey(raw)) {
                return envVariables.get(raw) + " /* env variable " + raw + " replaced */";
            }


            // Replace constructor parameter names with state variable names
            // Example: _id -> id
            if (cInfo != null && cInfo.paramToStateMapping != null &&
                    cInfo.paramToStateMapping.containsKey(raw)) {
                raw = cInfo.paramToStateMapping.get(raw);
            }
    ...
    private ConstructorInfo cInfoFor(String cls, Map<String, String> inst2cls) {
        for (ReactiveClassDeclaration rcd : classes) {
            if (rcd.getName().equals(cls)) {
                // Build fresh each time to avoid partial state caching
    ...
    /**
     * Generate the reaction body for an unused msgsrv (for documentation purposes).
     */
    private ReactionContent generateUnusedMsgsrvReactionBody(
            MsgsrvDeclaration unusedMsgSrvDecl,
            String className,
            ConstructorInfo cInfo,
            Map<String, String> instanceToClass,
            Map<String, List<CallDetail>> callsByInstance
        ...
        ReactionContent rc = new ReactionContent();
        if (unusedMsgSrvDecl == null || unusedMsgSrvDecl.getBlock() == null) {
            return rc;
        }
